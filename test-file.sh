...
function log() {
    ...
    echo 1>&2 -e "${logtime}\t${level}\t${msg}\t{${attrs}}"
}
...
function influxd::config::get() {
    ...
    *.yaml | *.yml) local influxd_config_format=yaml ;;
    ...
    if dasel -r "${influxd_config_format}" delete "${primary_key}" <"${INFLUXD_CONFIG_PATH}" &>/dev/null; then
...
function init_influxd() {
    ...
    if [ -n "$(influxd::config::get tls-cert "${@}")" ] &&
        [ -n "$(influxd::config::get tls-key "${@}")" ]; then
